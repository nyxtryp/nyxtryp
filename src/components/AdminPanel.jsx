...
export default function AdminPanel() {
  ...
  const upload = async e => {
    ...
    if (tooLarge) {
      setError(`«${tooLarge.name}» больше 4 МБ. Большие MP3 подключим отдельным механизмом напрямую в GitHub.`)
      return
    }

    setBusy(true)
    setStatus(selected.length > 1 ? `СОХРАНЕНИЕ 0/${selected.length}…` : 'СОХРАНЕНИЕ…')
    setError('')
    try {
