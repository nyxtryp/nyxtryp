...
export default function TracksPlayer({ onClose }) {
  ...
  const setupAudio = async () => {
    ...
    vuLeftDataRef.current = new Uint8Array(left.fftSize)
    vuRightDataRef.current = new Uint8Array(right.fftSize)
    spectrumDataRef.current = new Uint8Array(spectrum.frequencyBinCount)
  ...
  useEffect(()=>{
    ...
    const drawVU=()=>{
      ...
      if(!split){raf=requestAnimationFrame(drawVU);return}
      const lt=vuLeftDataRef.current,rt=vuRightDataRef.current
      if(!lt||!rt){raf=requestAnimationFrame(drawVU);return}
  ...
  useEffect(()=>{
    ...
    const drawSpectrum=()=>{
      ...
      if(split){
        ...
        const ld=spectrumDataRef.current
        if(!ld){raf=requestAnimationFrame(drawSpectrum);return}
        ...
        for(let i=0;i<bars;i++){
          ...
          const v=Math.max(.002,smoothed[i]),h=Math.max(3,maxBarHeight*v),x=pad+i*(barWidth+gap),y=baseY-h
  ...
  return <>
    <style>{`
      ...
      .tracks-overlay{position:fixed;inset:0;z-index:100;display:flex;align-items:center;justify-content:center;background:rgba(0,0,0,.1)}
