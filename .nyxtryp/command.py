...
# Remove the temporary test marker/text if present.
s = s.replace('<div style={{color:"white",fontSize:"24px",fontWeight:"bold",padding:"8px 0"}}>ПРИВЕТ</div>\n\n          ', '')

needle = '<div className="tracks-vu-wrap">'
controls = '<div className="tracks-controls">'

if needle not in s:
    raise SystemExit("tracks-vu-wrap not found")

pattern = re.escape(needle) + r'.*?' + re.escape(controls)
                ...
                <div className="tracks-mc-meter" key={channel}>
                  ...
                  <svg className="tracks-mc-gauge" viewBox="0 0 360 155" preserveAspectRatio="xMidYMid meet">
                    ...
                    {[-40,-30,-20,-10,0,10,20].map(db => {
                      ...
                      return (
                        <g key={db}>
                          <line x1={x1} y1={y1} x2={x2} y2={y2} stroke="rgba(225,238,248,.7)" strokeWidth="1.5" />
                          <text x={tx} y={ty} textAnchor="middle" fill="rgba(225,238,248,.72)" fontSize="10">{db}</text>
                        </g>
                      )
                    })}
...
if count != 1:
    raise SystemExit("VU block replacement failed")
